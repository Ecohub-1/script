...
getgenv().PromptRadius = 40 -- ระยะหา prompt
...
local function UniversalOptimize()
    ...
    safe(function()
        ...
        pcall(function() if Lighting:FindFirstChild("Brightness") then Lighting.Brightness = 0 end end)
        ...
        local function trimParticles(parent)
        ...
        trimParticles(workspace)
        trimParticles(game:GetService("Players").LocalPlayer.Character or game:GetService("Players").LocalPlayer)
...
Tabs.MVP:Button({
    Title = "🚀 ลดแลค + ป้องกันหลุด AFK",
    Desc = "ลดแลคขั้นสูง (ฟามเท่านั้น)",
    Callback = function()
        setfpscap(1000)
        UniversalOptimize()
    end
...
RunService.RenderStepped:Connect(function()
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            if ESP.Enabled and root and humanoid and humanoid.Health > 0 then
...
Tabs.MVP:Toggle({ Title = "เปิด ESP", Default = false, Callback = function(v) ESP.Enabled = v end })
Tabs.MVP:Input({
    Title = "ระยะมอง ESP",
    ...
    Callback = function(text)
        ...
        if num then ESP.MaxDistance = math.clamp(num,100,10000) end
    end
})
Tabs.MVP:Toggle({ Title = "สีรุ่ง Rainbow", Default = false, Callback = function(v) ESP.Rainbow = v end })
Tabs.MVP:Toggle({ Title = "แสดงเฉพาะศัตรู", Default = false, Callback = function(v) ESP.EnemyOnly = v end })
Tabs.MVP:Toggle({ Title = "แสดงกล่อง", Default = true, Callback = function(v) ESP.ShowBox = v end })
Tabs.MVP:Toggle({ Title = "แสดงชื่อ", Default = true, Callback = function(v) ESP.ShowName = v end })
Tabs.MVP:Toggle({ Title = "แสดง HP", Default = true, Callback = function(v) ESP.ShowHealth = v end })
Tabs.MVP:Toggle({ Title = "แสดงระยะ", Default = true, Callback = function(v) ESP.ShowDistance = v end })
Tabs.MVP:Toggle({ Title = "แสดงเส้น", Default = true, Callback = function(v) ESP.ShowTracer = v end })
